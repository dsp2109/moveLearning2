...
def pok_learn(env,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for t in range(max_timesteps):
            ...
            #act(np.array(obs)[None], update_eps=update_eps, **kwargs)[0] #DP this is what we replace - what does act do??
            #action = np.int64(env.action_space.sample()) 

            action = np.int64(env._get_action())
            ...
            # if t > learning_starts + 2003:
            #     mean_1000batch_tderror = round(np.mean(td_error_list[-4000:-1]),1)
            # else:
            #     mean_1000batch_tderror = -1
            ...
            if done and print_freq is not None and num_episodes % print_freq == 0:
                ...
                # logger.record_tabular("mean 100 episode reward (how to interpret)?", round(np.mean(episode_rewards[-101:-1]), 1))
                # if t > 10000:
                #     logger.record_tabular("mean abs 1000 td errs", mean_1000batch_tderror) #DP
            ...
            if (checkpoint_freq is not None and t > learning_starts and
                ...
                #                    saved_mean_reward, mean_1000batch_tderror))
        ...
        U.save_state(model_file)
        logger.log("Saved with total steps {}".format(max_timesteps))
